...
Call `env()` only from configuration files. After configuration is cached, LaraGram does not load the application's `.env` file, so application code should read configuration values through `config()`.
...
Do not commit plaintext production secrets. LaraGram can encrypt an environment file so its encrypted form can be stored safely, while deployment platforms can supply secrets through their native secret stores.
...
```bash
php laragram env:encrypt --env=production --readable
php laragram env:decrypt --env=production
```
