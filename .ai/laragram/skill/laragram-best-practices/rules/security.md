...
Define `$fillable` when a model is populated from request-derived arrays, or deliberately guard attributes by another consistent model convention. LaraGram models guard all attributes by default; `$guarded = []` opts out of that protection.
...
Include `@csrf` in state-changing Blade forms handled by LaraGram's `web` middleware. Routes intentionally excluded from cross-site request forgery (CSRF) verification, such as validated third-party webhooks, need their own authenticity check.
...
Luna's HTTP client returns the encrypted `XSRF-TOKEN` cookie in the `X-XSRF-TOKEN` header. Confirm equivalent configuration when using another HTTP client. Do not disable CSRF protection merely to fix a token mismatch.
...
Validate expected content type, dimensions where relevant, and size. LaraGram's `mimes` rule reads the file contents and guesses a Multipurpose Internet Mail Extensions (MIME) type corresponding to the listed extensions; it does not validate the user-assigned filename extension. The `extensions` rule checks that extension and should not be used by itself.
...
Use LaraGram's storage methods to generate a filename, and store untrusted files outside a publicly executable location. Public files can require additional controls, such as image re-encoding, content-disposition headers, and explicit blocking of active formats.
...
## Protect the Webhook and Bot Credentials

Set a unique `secret_token` for every bot connection in `config/bot.php` and register it with the webhook, so updates that do not carry Telegram's `X-Telegram-Bot-Api-Secret-Token` header can be told apart from forged requests (`$request->secretToken()`). Keep bot tokens, `API_ID`/`API_HASH`, and MTProto session files in `.env` or storage only, never in code, logs, templates, or frontend bundles.
...
Callback data comes from buttons the bot sent, but a client can send any string. Constrain listen parameters (`whereNumber()`, `whereIn()`), check that the referenced record belongs to the current user, and answer invalid presses gracefully.
