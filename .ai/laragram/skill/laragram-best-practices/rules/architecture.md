...
Extract a discrete business operation into an action class when doing so makes the operation easier to reuse or test. An action class has no special meaning to LaraGram; follow the project's naming and invocation conventions.
...
When no LaraGram helper exists, prefer multibyte-aware functions such as `mb_strlen()` and `mb_strtolower()` for UTF-8 text. For example, `strlen()` counts bytes, while `strtolower()` is not multibyte-aware.
...
// Prefer LaraGram's Str helpers when available
Str::length('José');       // 4
...
Run independent operations concurrently through LaraGram's configured concurrency driver.
...
With a process-based driver, each closure runs in a separate PHP process that boots the application. Use concurrency when independent database queries, HTTP client calls, or computations benefit enough to offset process and serialization overhead. The `sync` driver executes closures sequentially.

## Follow Framework Conventions

Follow LaraGram conventions unless the domain or an existing schema requires an override.
