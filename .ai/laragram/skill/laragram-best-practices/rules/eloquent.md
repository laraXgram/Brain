...
Cast a date or timestamp attribute when application code should treat it as a Tempora instance. Eloquent already casts the conventional `created_at` and `updated_at` timestamps.

Manual parsing in the template:

```blade
{{ Tempora::parse($order->ordered_at)->toDateString() }}
```
