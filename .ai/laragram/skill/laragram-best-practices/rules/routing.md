...
Let LaraGram resolve models from route parameters when the default lookup and missing-model behavior fit the endpoint.
...
Use `Route::resource()` or `Route::apiResource()` when the endpoint follows LaraGram's resource-controller actions. Define explicit routes when the behavior does not fit that vocabulary.
...
As a general default, organize each controller around one resource and use LaraGram's standard resource actions: `index`, `show`, `create`, `store`, `edit`, `update`, and `destroy`. This keeps routes predictable and prevents controllers from accumulating unrelated behavior.
