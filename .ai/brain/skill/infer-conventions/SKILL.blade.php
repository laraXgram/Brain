...
description: "Use this skill to analyze how a LaraGram application is actually written and record its conventions as shared rules. Trigger when the user wants to detect, infer, document, or standardize project conventions or coding style, set up or grow `.ai/rules`, resolve mixed or conflicting patterns (e.g. \"do we reply with templates or inline sendMessage calls?\"), or onboard agents and teammates to \"how we do things here\". Covers: a systematic sweep of LaraGram convention dimensions (bot listens, replies, keyboards, templates, conversations, validation, models, architecture, web and Luna frontend, database, console), open-ended house-pattern discovery, conflict reporting, and recording rules scoped to the right paths via the Brain `record-rule` MCP tool. Only run this skill when the user explicitly asks for it; never start a sweep as part of another task. Do not use for one-off code review, enforcing formatting a linter already handles, or editing `.ai/rules` files by hand."
...
Learn how this application writes LaraGram, then record what you learn as durable, path-scoped rules other agents will read. You are documenting reality, not improving it.
...
Read `composer.json` (installed packages tell you which checklist groups apply), `bootstrap/app.php` (which listen and route files exist), the formatter / static-analysis config, `.ai/rules/index.md` if present, and most important, map the `app/` tree and `listens/`. List every directory under `app/` (and any `Modules/`, `src/`, `packages/`, or domain root). Every folder beyond LaraGram's default skeleton (`Controllers`, `Models`, `Providers`) and the generator folders (`Console`, `Conversations`, `templates`, `Middleware`, `Exceptions`, `Http`) is a structural pattern the app committed to and a high-value rule waiting to be written: `Actions`, `Services`, `Data` or DTOs, `Queries`, `Repositories`, `ViewModels`, `Pipelines`, `Support`, `Enums`, `Contracts`, `Observers`, or `Domain` and module roots. Note each one. You will confirm how it is used in Step 2.

@if($assist->hasPackage('laraxgram/luna'))
This app ships Luna, so the web and Luna frontend checklist group applies. Sweep it.
@else
This app has no Luna package installed. Confirm from `routes/` and `resources/views` whether it serves web pages at all before spending time on the frontend group.
@endif
...
First, close out the architecture map from Step 0. For every non-default `app/` directory you listed, confirm how the pattern is used and apply the same evidence and decisions-not-defaults tests as Step 1. Generator-standard or sparsely used directories such as `Rules`, `Observers`, and `Policies` are signals to inspect, not automatic conventions. Make genuine structural patterns candidates: Action classes invoked via `handle` / `execute` / `__invoke`, Services constructor-injected, `Queries` objects exposing `builder(): Builder`, DTOs as readonly classes, module or domain folders as the unit of organization. Scope each qualifying pattern to its own directory glob. Also record a consistent deliberate absence, such as "no repository layer, controllers query Eloquent directly", so the next agent matches the app's altitude.
...
- Bot listens and bot controllers: `listens/**`, `app/Controllers/**`.
- Templates and conversations: `app/templates/**`, `app/Conversations/**`.
- Web controllers, routing, validation, responses: `app/Http/**`, `routes/**`, or `app/Modules/*/Http/**` when each module owns its HTTP layer.
- Luna pages: `resources/js/**`.
