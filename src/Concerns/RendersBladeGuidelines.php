...
namespace LaraGram\Brain\Concerns;

use LaraGram\Support\Facades\Blade;
use LaraGram\Brain\Install\GuidelineAssist;
use LaraGram\Brain\Support\Fences;
...
trait RendersBladeGuidelines
{
    ...
    protected function renderContent(string $content, string $path, array $data = []): string
    {
        ...
        // Temporarily replace backticks, PHP opening tags, component tags, and Blade/Temple8 directives
    ...
    protected function processBrainSnippets(string $content): string
    {
        return preg_replace_callback('/(?<!@)@brainsnippet\(\s*(?P<nameQuote>[\'"])(?P<name>[^\1]*?)\1(?:\s*,\s*(?P<langQuote>[\'"])(?P<lang>[^\3]*?)\3)?\s*\)(?P<content>.*?)@endbrainsnippet/s', function (array $matches): string {
            $name = $matches['name'];
            $lang = empty($matches['lang']) ? 'html' : $matches['lang'];
            $snippetContent = trim($matches['content']);

            $placeholder = '___BRAIN_SNIPPET_'.count($this->storedSnippets).'___';
    ...
    {
        $content = $this->processBrainSnippets($content);
