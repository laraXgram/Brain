...
use LaraGram\Brain\Install\Detection\DetectionStrategyFactory;
use LaraGram\Brain\Install\Enums\McpInstallationStrategy;
use LaraGram\Brain\Install\Enums\Platform;
use LaraGram\Brain\Install\Mcp\FileWriter;
use LaraGram\Brain\Install\Mcp\TomlFileWriter;
...
abstract class Agent
{
    ...
    {
        $phpBinaryPath = config('brain.executable_paths.php') ?: 'php';
    ...
    public function getCommanderPath(bool $forceAbsolutePath = false): string
    {
        return ($this->useAbsolutePathForMcp() || $forceAbsolutePath) ? base_path('laragram') : 'laragram';
    }
    ...
    public static function fromName(string $name): ?Agent
    {
        ...
        $brainManager = app(BrainManager::class);

        foreach ($brainManager->getAgents() as $class) {
