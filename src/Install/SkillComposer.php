...
use LaraGram\Brain\Install\Concerns\DiscoverPackagePaths;
...
use LaraGram\Brain\Support\Yaml\Exceptions\ParseException;
use LaraGram\Brain\Support\Yaml\Yaml;
...
class SkillComposer
{
    ...
    /**
     * Get all discovered skills (Brain built-in, third-party, and user).
     *
    ...
    /**
     * Skills Brain ships itself, discovered from `.ai/brain/skill`.
     *
     * @return Collection<string, Skill>
     */
    protected function getCoreSkills(): Collection
    {
        return $this->discoverSkillsFromDirectory(
            $this->getBrainAiPath().DIRECTORY_SEPARATOR.'brain'.DIRECTORY_SEPARATOR.'skill',
            'brain',
        );
    }

    /**
     * Deployment skills, discovered from `.ai/deployments/skill`.
     *
     * @return Collection<string, Skill>
     */
    ...
    {
        return $this->discoverSkillsFromDirectory(
            $this->getBrainAiPath().DIRECTORY_SEPARATOR.'deployments'.DIRECTORY_SEPARATOR.'skill',
            'deployments',
        );
    }
     ...
     * @return Collection<string, Skill>
     */
    protected function getBrainSkills(): Collection
    {
        ...
        $skills = $this->packages()
            ...
            ->flatMap(function (Package $package): Collection {
                ...
                $vendorSkillPath = $this->resolveFirstPartyBrainPath($package, 'skills');

                $vendorSkills = $vendorSkillPath !== null
                    ? $this->discoverSkillsFromDirectory($vendorSkillPath, $name)
                    : collect();

                $aiPath = $this->getBrainAiPath().DIRECTORY_SEPARATOR.$name;
