...
[Vite](https://vitejs.dev) is a modern frontend build tool that provides an extremely fast development environment and bundles your code for production. When building applications with LaraGram, you will typically use Vite to bundle your application's CSS and JavaScript files into production-ready assets.

LaraGram integrates seamlessly with Vite by providing an official plugin and Blade directive to load your assets for development and production.
...
You must ensure that Node.js (16+) and NPM are installed before running Vite and the LaraGram plugin:
...
You can easily install the latest version of Node and NPM using simple graphical installers from [the official Node website](https://nodejs.org/en/download/). Or, if you are using [LaraGram Armada](https://laraxgram.github.io/armada), you may invoke Node and NPM through Armada:

```shell
./vendor/bin/armada node -v
./vendor/bin/armada npm -v
```

<a name="installing-vite-and-laragram-plugin"></a>
### Installing Vite and the LaraGram Plugin

Within a fresh installation of LaraGram, you will find a `package.json` file in the root of your application's directory structure. The default `package.json` file already includes everything you need to get started using Vite and the LaraGram plugin. You may install your application's frontend dependencies via NPM:
...
The LaraGram Vite plugin requires you to specify the entry points for your application. These may be JavaScript or CSS files, and include preprocessed languages such as TypeScript, JSX, TSX, and Sass.
...
If you are building an SPA, including applications built using Luna, Vite works best without CSS entry points:
...
#### Running the Development Server in Armada on WSL2

When running the Vite development server within [LaraGram Armada](/armada) on Windows Subsystem for Linux 2 (WSL2), you should add the following configuration to your `vite.config.js` file to ensure the browser can communicate with the development server:
...
If you are running the development server in [Armada](/armada) on WSL2, you may need some [additional configuration](#configuring-hmr-in-sail-on-wsl2) options.
...
By default, The LaraGram plugin provides a common alias to help you hit the ground running and conveniently import your application's assets:
...
You may then include the plugin in your `vite.config.js` configuration file. There are a few additional options you will need when using the Vue plugin with LaraGram:
...
export default defineConfig({
    plugins: [
        laragram(['resources/js/app.js']),
        vue({
            template: {
                transformAssetUrls: {
                    // The Vue plugin will re-write asset URLs, when referenced
                    // in Single File Components, to point to the LaraGram web
                    // server. Setting this to `null` allows the LaraGram plugin
...
The LaraGram Vite plugin provides a convenient `resolvePageComponent` function to help you resolve your Luna page components. Below is an example of the helper in use with Vue 3; however, you may also utilize the function in other frameworks such as React or Svelte:

```js
import { createApp, h } from 'vue';
import { createLunaApp } from '@laraxgram/vue3';
import { resolvePageComponent } from 'laragram-vite-plugin/luna-helpers';
...
All LaraGram applications already include Tailwind and a properly configured `vite.config.js` file. So, you only need to start the Vite development server or run the `dev` Composer command, which will start both the LaraGram and Vite development servers:
...
import { defineConfig } from 'vite';
import laragram from 'laragram-vite-plugin';
import { google } from 'laragram-vite-plugin/fonts';
...
The LaraGram Vite plugin includes provider helpers for Google Fonts, Bunny Fonts, Fontsource, and local fonts:

```js
import { defineConfig } from 'vite';
import laragram from 'laragram-vite-plugin';
import { bunny, fontsource, google, local } from 'laragram-vite-plugin/fonts';
...
Optimized fallbacks require the `fontaine` package, which is not installed by default. If you want LaraGram to generate metric-adjusted fallback font faces, you should install `fontaine` as a development dependency:

```shell
npm install --save-dev fontaine
```

If `fontaine` is not installed or cannot read a font file, LaraGram will skip the optimized fallback for that font and continue using any fonts configured via the `fallbacks` option.
...
However, to accomplish this, you need to make Vite aware of your assets by specifying them in the plugin's `assets` option. This option is intended for static files that you want to reference directly with `Vite::asset`. If you want LaraGram to generate font CSS and preload links, use the [`fonts` option](#working-with-fonts) instead.
...
> [!NOTE]
> Prior to version 3 of the LaraGram Vite plugin, static assets had to be imported in your application's entry point using `import.meta.glob`. The `assets` option was introduced due to changes in Vite 8.
...
Under the hood, the LaraGram Vite plugin uses the [vite-plugin-full-reload](https://github.com/ElMassimo/vite-plugin-full-reload) package, which offers some advanced configuration options to fine-tune this feature's behavior. If you need this level of customization, you may provide a `config` definition:
...
```blade
<img src="{{ Vite::image('logo.png') }}" alt="LaraGram Logo">
```
...
LaraGram's Vite integration will attempt to resolve your assets while running your tests, which requires you to either run the Vite development server or build your assets.

If you would prefer to mock Vite during testing, you may call the `withoutVite` method, which is available for any tests that extend LaraGram's `TestCase` class:
...
use LaraGram\Foundation\Testing\TestCase as BaseTestCase;
...
The LaraGram Vite plugin makes it painless to set up server-side rendering with Vite. To get started, create an SSR entry point at `resources/js/ssr.js` and specify the entry point by passing a configuration option to the LaraGram plugin:
...
class AddContentSecurityPolicyHeaders
{
     ...
     *
     * @param  \Closure(\LaraGram\Http\Request): (\LaraGram\Http\BaseResponse)  $next
     */
...
If you already have a nonce that you would like to instruct LaraGram to use, you may pass the nonce to the `useCspNonce` method:
...
Out of the box, LaraGram's Vite plugin uses sensible conventions that should work for the majority of applications; however, sometimes you may need to customize Vite's behavior. To enable additional customization options, we offer the following methods and options which can be used in place of the `@vite` Blade directive:
...
If you are experiencing Cross-Origin Resource Sharing (CORS) issues in the browser while fetching assets from the Vite dev server, you may need to grant your custom origin access to the dev server. Vite combined with the LaraGram plugin allows the following origins without any additional configuration:
...
The easiest way to allow a custom origin for your project is to ensure that your application's `APP_URL` environment variable matches the origin you are visiting in your browser. For example, if you visiting `https://my-app.laragram`, you should update your `.env` to match:

```env
APP_URL=https://my-app.laragram
```
...
You may also include regex patterns, which can be helpful if you would like to allow all origins for a given top-level domain, such as `*.laragram`:
...
Some plugins within the Vite ecosystem assume that URLs which begin with a forward-slash will always point to the Vite dev server. However, due to the nature of the LaraGram integration, this is not the case.
